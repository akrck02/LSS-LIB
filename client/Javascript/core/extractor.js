...
export function extract(json) {
  if (exists(json.type)) {
    switch (json.type) {
      case "component":
        return extract_component(json);
      case "setting":
        return extract_setting(json);
      case "theme":
        return extract_theme(json);
      case "variable set":
        return extract_variable_set(json);
      default:
        throw "[LSS][ERROR] Unknown file type: " + json.type;
    }
...
function extract_component(json) {
  ...
  if (exists(json.styles))
    for (const key in json.styles) {
      ...
      if (is_variable(value) && exists(component.variables[value.substr(2)]))
        value = component.variables[value.substr(2)].css_var();
  ...
  if (exists(json.actions))
    for (const key in json.actions) {
      let ac = new Action(key, json.name);
      for (const style in json.actions[key]) {
        ac._style(new Style(style, json.actions[key][style]));
      }
      component._action(ac);
    }
